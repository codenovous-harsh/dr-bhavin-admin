...
export function SimulationResultColumn({ title, loading, result, onRetry }: Props) {
  ...
  return (
    <div className="flex flex-col rounded border p-4">
      ...
      {loading && <div className="py-8 text-center text-gray-500">Running…</div>}
      ...
      {!loading && result?.response && (
        <>
          ...
          <div className="prose prose-sm max-w-none overflow-auto">{renderMarkdown(result.response)}</div>
          <button
